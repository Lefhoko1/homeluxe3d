...
create unique index shop_subscriptions_one_active
...
create index product_variants_material on product_variants (material_name)
...
create unique index placements_one_live_per_slot
  on placements (slot_id) where status = 'live' and slot_id is not null;

create index placements_scene_live on placements (scene_id) where status = 'live';
create index placements_shop on placements (shop_id);
...
create index interaction_events_shop_time on interaction_events (shop_id, occurred_at desc);
create index interaction_events_placement on interaction_events (placement_id);
...
create index enquiries_shop on enquiries (shop_id, created_at desc);
