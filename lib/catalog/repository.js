...
export function rowsToManifest(rows, sceneSlug) {
  ...
  rows.forEach((row) => {
    ...
    // A finish has no position -- it dresses a surface rather than standing in
    // the room, so it belongs in the product list but not the placement list.
    if (row.model_url) {
      placements.push({
        product: row.qualified_id,
        room: row.room_code ?? "unassigned",
        position: toThreePosition(row.x_mm, row.y_mm, row.z_mm),
