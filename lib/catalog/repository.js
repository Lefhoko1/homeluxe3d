...
import { mmToThree } from "../scene/transforms.js";
  ...
  rows.forEach((row) => {
    ...
    if (!shop.products.has(row.qualified_id)) {
      ...
      shop.products.set(row.qualified_id, {
        ...
        // THE MODULE THE SURFACE REPEATS AT, from the VARIANT.
        //
        // The finish spec used to take this from the product's overall width,
        // which is right for a tile by coincidence -- a 600mm tile is 600mm
        // wide -- and wrong for anything whose repeat is not its size. A roll
        // of diamond mesh is 30 metres wide and repeats every 50mm, so the
        // product's width made it thirty metres between one diamond and the
        // next. The variant is the thing that knows.
        textureTileMm: row.texture_tile_mm ?? undefined,
