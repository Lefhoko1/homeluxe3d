...
/**
 * The scene's catalogue, from the database.
 *
 * THROWS rather than returning something plausible. Each of the three ways
 * this can fail is a different thing being wrong, and each says so, because
 * "no products in the house" on its own sends you looking at the 3D scene --
 * which is the one place the fault will never be.
 *
 * @throws {Error} unconfigured, unreachable, or not seeded
 * @returns {Promise<{source: string, shops: Array, houses: object}>}
 */
export async function fetchSceneCatalog({ scene = "3bed" } = {}) {
  ...
  if (!data?.length) {
    // Distinguished from a read failure on purpose: the connection worked and
    // the answer was "nothing", which almost always means the seed was never
    // applied. Naming the command is the difference between a dead end and a
    // fix.
    throw new Error(
  ...
  return rowsToManifest(data, scene);
}
