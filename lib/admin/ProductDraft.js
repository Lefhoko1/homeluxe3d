...
export class ProductDraft {
  ...
  async save(onProgress = () => {}) {
    ...
    try {
      ...
      const model = await store.putModel(this.modelFile, {
        shopSlug: this.shopSlug,
        productSlug: slug,
        variantSlug: "default",
      });

      // A FILE IN A BUCKET IS NOT AN ASSET. Uploading with upsert:true
      // overwrites the previous model in place, so before this the old one
      // was simply gone and nothing could tell a broken export from one that
      // had not finished processing. register_asset makes the upload a
      // NUMBERED VERSION, and carries the bounding box ModelInspector already
      // measured in the browser so the database has something to check
      // against. See migrations 0011 and 0012.
      onProgress("Recording the upload…");
      const { data: version, error: assetError } = await supabase.rpc(
        "register_asset",
        {
          p_shop_slug: this.shopSlug,
          p_kind: "model",
          p_name: this.name.trim(),
          p_slug: slug,
          p_storage_path: model.path,
          p_mime: model.mime,
          p_bytes: model.bytes,
          p_width_mm: this.inspection?.dimensions?.width ?? null,
          p_depth_mm: this.inspection?.dimensions?.depth ?? null,
          p_height_mm: this.inspection?.dimensions?.height ?? null,
          p_triangles: this.inspection?.triangles ?? null,
        }
      );
      if (assetError) throw new Error(explain(assetError));
      ...
      // -- 4b. is it actually the right size? -----------------------------
      // Now, and not before, because the check compares the model against
      // what the shop SAID the product measures -- and that comparison needs
      // the variant to exist to join through. A model exported in
      // centimetres arrives a hundred times too small and looks perfectly
      // fine on its own; you notice when the sofa is the size of a shoe.
      onProgress("Checking the model…");
      const { data: assetProblems, error: checkError } = await supabase.rpc(
        "validate_asset_version",
        { p_version: version.id }
      );
      if (checkError) throw new Error(explain(checkError));
      const problemsFound = assetProblems ?? [];

      // -- 5. publish ----------------------------------------------------
      // Last, so the product becomes visible only once it is complete -- and
      // NOT AT ALL if the model failed its check. The upload is kept either
      // way: the file is fine, the export was wrong, and throwing the work
      // away would only mean uploading it again. It stays a draft, which is
      // invisible to v_live_placements, so a model in the wrong units can be
      // fixed without ever having reached a visitor.
      const patch = { status: problemsFound.length ? "draft" : this.status };
