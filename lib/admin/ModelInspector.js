...
import { createGltfLoader } from "../gltf/decoders";
...
export async function inspectModel(file) {
  ...
  // THE SAME LOADER THE HOUSE USES, decoders and all. When this was built
  // with only Draco, a meshopt-compressed upload was rejected here with a
  // message blaming the file -- and it would have been just as invisible
  // in the house, because the runtime loaders were missing the decoder
  // too. Inspecting with anything less capable than the thing that will
  // render it turns a supported file into a rejected one.
  ...
  try {
      ...
      loader.load(url, resolve, undefined, (error) =>
        reject(
          new Error(
            ...
            `Draco and meshopt compression are both supported, so that is not ` +
            `the cause. ` +
            `(${error?.message ?? error})`
          )
        )
    ...
    // NOT `draco.dispose()`. The inspector used to own its decoder and threw
    // it away afterwards; the decoders are shared with the renderer now, so
    // disposing here would shut down the Web Workers the house itself is
    // using -- and the reference to a local `draco` no longer existed at all,
    // which is what "draco is not defined" was. Nothing here owns them, so
    // nothing here disposes them.
  }
}
