...
export class AdminData {
  ...
  async placements({ sceneSlug = "3bed" } = {}) {
    return this.#rows(
      this.#db()
        .from("placements")
        .select(`
          id, status, note, x_mm, y_mm, rotation_deg, showcase_rank,
  ...
  /**
   * Put a placement on the front page, or take it off.
   *
   * THROUGH AN RPC, not an update, and the difference is a permission. A shop
   * manager may edit their own placements -- that is their furniture -- and
   * Postgres has no per-column row-level security, so any policy loose enough
   * to let them straighten their own sofa would also let them put themselves
   * on the front page. `set_showcase_rank` checks `is_platform_admin()` and
   * refuses everyone else. See migration 0021.
   *
   * @param {string} placementId
   * @param {number|null} rank  1, 2, 3... in order; null to remove it
   */
  async setShowcaseRank(placementId, rank) {
    const { data, error } = await this.#db().rpc("set_showcase_rank", {
    ...
    if (error) {
      throw new Error(
        error.code === "42501"
          ? "Only a platform admin can choose what leads the front page."
          : `Could not change the front page: ${error.message}`
      );
    }
  ...
  /**
   * Who is actually in the house, by day.
   *
   * `interaction_events.user_id` was on the table from the start and nothing
   * ever set it, so every session looked anonymous -- including the ones
   * belonging to somebody signed in at the time. Migration 0019 fills it from
   * the token, which is what makes "how many of our visitors are known"
   * answerable at all.
   *
   * COUNTS, NOT NAMES. How many people came is an operator's question; who
   * they were is a different one, and this view does not answer it.
   */
  async traffic(days = 30) {
    return this.#rows(
      this.#db()
        .from("v_daily_traffic")
        .select("day, sessions, known_sessions, people, events, views, clicks, enquiries")
        .order("day", { ascending: false })
        .limit(days),
      "the traffic"
    );
  ...
  /** New accounts by day, so "is the audience growing" has an answer. */
  async signups(days = 30) {
    return this.#rows(
      this.#db()
        .from("v_signups")
        .select("day, accounts, staff")
        .order("day", { ascending: false })
        .limit(days),
      "the sign-ups"
    );
  ...
  /** Enquiries with the conversation summarised. */
  async enquiryThreads() {
    return this.#rows(
      this.#db()
        .from("v_enquiry_threads")
        .select("id, shop_name, product_name, from_name, email, phone, message, status, created_at, replies, last_reply, last_reply_at")
        .order("created_at", { ascending: false })
        .limit(100),
      "the enquiries"
    );
   ...
   * Answer a customer.
   *
   * `from_shop` is written explicitly rather than inferred from membership,
   * because membership changes: somebody who leaves next month still wrote
   * their replies as the shop, and a thread that re-attributes them later is
   * a thread that lies about what was said.
   *
   * A trigger moves the enquiry to `replied`, and another tells the customer
   * -- in the app and through the same email outbox everything else uses.
   */
  async replyToEnquiry(enquiryId, authorId, body) {
    if (!body?.trim()) throw new Error("Write the reply first.");
    return this.#one(
